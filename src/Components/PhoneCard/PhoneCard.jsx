...
const PhoneCard = ({phone ,deletable , handleDelete}) => {
  ...
  {deletable && (
    <div onClick={() => handleDelete(id)}  className='absolute -top-2 -right-2 bg-gray-900 p-3 ml-5 rounded-full hover:bg-gray-300 group cursor-pointer' ><FaDeleteLeft size={15} className='text-gray-100 group-hover:text-gray-900'/>
</div>)}
</div>
    );
};
